...
async function loadContent(): Promise<SiteContent> {
  ...
  try {
    ...
    const news: ArticleData[] = result.articles?.length
...
function Home({settings, axes, news}:{settings:Settings; axes:AxisData[]; news:ArticleData[]}) { return <><Header /><main>
...
function Article({slug, news}:{slug:string; news:ArticleData[]}) { const n=news.find(x=>x.slug===slug); if(!n) notFound(); return <Shell><section className="article-hero"><div className="container"><p className="eyebrow">{n.category}</p><h1>{n.title}</h1><p className="lead">{n.text}</p></div></section><div className="article-image"><Image src={n.image} alt={n.imageAlt} fill priority quality={95} sizes="100vw" /></div><section className="section section-light"><div className="container narrow">{n.body.map((p,i)=><p className="large-copy detail-copy" key={i}>{p}</p>)}</div></section></Shell>; }
